...
from dataset import DEMSuperResolutionDataset, collect_valid_samples
...
class Validator:
    ...
    def validate_dataset(
        ...
        for batch_idx, batch in enumerate(tqdm(val_loader, desc="Validating")):
            ...
            for i in range(hrdem.shape[0]):
                ...
                # 计算指标
                rmse = torch.sqrt(nn.MSELoss()(pred, target)).item()
    ...
    def _save_prediction(
        ...
        usgs_np = usgs.squeeze().cpu().numpy()
        copernicus_np = copernicus.squeeze().cpu().numpy()
        
        # 保存为tif
        output_path = os.path.join(group_dir, f"{filename}_pred.tif")
        
        # 创建简单的transform（实际使用时应该使用原始数据的transform）
...
def main():
    ...
    parser.add_argument('--data_dir', type=str,
                        default=r"D:\研究文件\ResearchData\USA")
