...
class DemDownloader:
    ...
    def downloading_dem(self, dem, access_token):
        ...
        if not os.path.exists(self.dem_directory):
            os.makedirs(self.dem_directory, exist_ok=True)

        file_path = os.path.join(self.dem_directory, f"{dem}.zip")
        ...
        log.info("File %s downloaded (%s)", dem, total_size)  # 显示最终大小

        try:
            self.open_compressed_file(file_path, dem)
        except Exception as e:
            log.warning("Error in extracting the DEM file from the zip file, skipping: %s", e)
...
"""
python cdse_copernicus_dem_downloader.py ^
--geojson "C:\\Users\\Kevin\\Documents\\ResearchData\\GeoDAR_v10_v11\\USA_DAM_ByRegion\\group_1\\GeoDAR_v11_dams_of_USA_group1.geojson" ^
--output "C:\\Users\\Kevin\\Documents\\ResearchData\\GeoDAR_v10_v11\\USA_DAM_ByRegion\\group_1\\DEM" ^
--dem-type COPDEM30 ^
--username "2399898002@qq.com" ^
--password "CZ123456798cz!!"
